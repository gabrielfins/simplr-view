...
export function Input({leadingIcon, trailingIcon, label, helperText, invalidValueText, showInvalidValueIcon = true, showCharacterCount = false, ...props}: InputProps) {
    ...
    return (
        <div className={`input-component${isInputFocused ? ' focused' : ''}${isInputFilled ? ' filled' : ''}${leadingIcon ? ' leading' : ''}${trailingIcon ? ' trailing' : ''}${isInputedValueValid ? '' : ' invalid'}`}>
            <div>
                <div className="input-container">
                    ...
                    <input className="input" {...props} value={value} onInput={(evt) => handleInput(evt)} onFocus={() => setIsInputFocused(true)} onBlurCapture={() => setIsInputFocused(false)} />
                ...
                <div className="input-label-container">
                    <label htmlFor={props.id} className="input-label">{label}</label>
                </div>
            </div>
